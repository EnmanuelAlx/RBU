<template>
  <div>
    <v-toolbar flat color="#212121">
      <v-toolbar-title>Cantidad de veces que se reserva una sala</v-toolbar-title>
      <v-divider class="mx-2" inset vertical></v-divider>
      <v-spacer></v-spacer>

      <v-text-field v-model="search" append-icon="search" label="Search" single-line hide-details></v-text-field>

      <v-btn color="primary" dark class="mb-2" @click="filter = true">Filtrar Salas reservadas</v-btn>
    </v-toolbar>
    ...
    <v-data-table :headers="headers" :items="desserts" :search="search" class="elevation-1">
      <template v-slot:items="props">
        <td class="text-xs-left">{{ props.item.Sala_nombre }}</td>
        <td class>{{ props.item.CantidadReservas }}</td>
        <td
          class="text-xs-right"
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      {
        text: "Cantidad de veces solicitada",
        align: "center",
  ...
  methods: {
    initialize() {
      this.desserts = [];
      axios
        .get(`${this.$api}/reportes/reservas`)
        .then(res => {
    ...
    filtrar() {
      ...
      axios
        .post(`${this.$api}/reportes/reservas/fecha`, {
          fechaInicio,
          fechaFin
        })
        .then(res => {
            ...
            switch (elem.Sala_id_piso) {
              ...
              case 2:
                if (
                  ...
                  T3_1[elem.Day - 1] = elem.CantidadReservas;
                }
                if (
                  "12:00:00" < elem.HoraInicioReserva &&
                  elem.HoraInicioReserva <= "16:00:00"
                ) {
                  T3_2[elem.Day - 1] = elem.CantidadReservas;
                }
                if (
                  "16:00:00" < elem.HoraInicioReserva &&
                  elem.HoraInicioReserva <= "20:00:00"
                ) {
                  T3_3[elem.Day - 1] = elem.CantidadReservas;
                }
                break;
              ...
              case 3:
                if (
                  ...
                  TH_1[elem.Day - 1] = elem.CantidadReservas;
                }
                if (
                  "12:00:00" < elem.HoraInicioReserva &&
                  elem.HoraInicioReserva <= "16:00:00"
                ) {
                  TH_2[elem.Day - 1] = elem.CantidadReservas;
                }
            }
          }
        }); //final ForEach
