...
export class PersonasReservacion {
    ...
    @ManyToOne(() => {
        return Reservacion;
    }, (reservacion$) => reservacion$.personasReservacions, { onDelete: 'RESTRICT', onUpdate: 'RESTRICT' })
    @JoinColumn({ name: 'id_reservacion' })
    public idReservacion: Reservacion | null;

    @ManyToOne(() => {
        return Persona;
    }, (persona$) => persona$.personasReservacions, { onDelete: 'RESTRICT', onUpdate: 'RESTRICT' })
